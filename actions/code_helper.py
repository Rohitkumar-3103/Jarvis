        ...
        def generate_content(self, contents):
            ...
            for attempt in range(3):
                ...
                if res.status_code == 429:
                    ...
                    print(f"[Code] [API 429] Rate limited. Waiting {sleep_time}s before retry...")
            ...
            raise Exception("Gemini API rate limit exceeded (429). Please wait a moment and try again, Sir.")
...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    if output_path:
        p = Path(output_path)
        return p if p.is_absolute() else TEMP_DIR / p
    ext = ext_map.get((language or "python").lower(), ".py")
    return TEMP_DIR / f"jarvis_code{ext}"
...
- Do NOT use input() or wait for user keyboard/terminal input. Hardcode any demo parameters in the main block.
...
def _run_file(path: Path, args: list, timeout: int) -> tuple[str, bool]:
    interpreters = {
        ".py":  [sys.executable],
        ".js":  ["node"],
        ".ts":  ["ts-node"],
    ...
    if not interp:
        return f"No interpreter for {path.suffix}.", True
    ...
    try:
        ...
        if not has_error and error:
            error_lower = error.lower()
            error_signals = ["error", "exception", "traceback", "syntaxerror", "nameerror", "typeerror"]
            if any(s in error_lower for s in error_signals):
                has_error = True
                
        return output_str, has_error

    except subprocess.TimeoutExpired:
        return f"Timed out after {timeout}s.", True
    except FileNotFoundError:
        return f"Interpreter not found: {interp[0]}.", True
    except Exception as e:
...
def _build(description, language, output_path, args, timeout, speak=None, player=None) -> str:
    ...
    for attempt in range(1, MAX_BUILD_ATTEMPTS + 1):
        print(f"[Code] [Attempt] Attempt {attempt}/{MAX_BUILD_ATTEMPTS}")
        if player:
            player.write_log(f"[Code] Attempt {attempt}...")

        last_output, run_failed = _run_file(path, args, timeout)
        ...
        print(f"[Code] [Fixing] Error on attempt {attempt}, fixing...")
        if player:
...
def _run_action(file_path, args, timeout, player) -> str:
    ...
    output_str, _ = _run_file(p, args, timeout)
    return output_str
